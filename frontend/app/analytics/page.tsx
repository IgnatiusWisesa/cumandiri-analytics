...
import KreditUsiaProfesiTujuanTable from './KreditUsiaProfesiTujuanTable';
import UsiaPerTPTable from './UsiaPerTPTable';
import ProfesiPerTPTable from './ProfesiPerTPTable';
import UsiaKreditWithStatusTable from './UsiaKreditWithStatusTable';
import ProfesiKreditWithStatusTable from './ProfesiKreditWithStatusTable';
import TujuanPinjamanPembayaranTable from './TujuanPinjamanPembayaranTable';
  ...
  return (
    <div className="min-h-screen bg-neutral-50 p-6">
      <div className="mx-auto max-w-6xl space-y-10">
                        ...
                        >
                          ...
                          {JENIS_LIST.map((j) => {
                            ...
                            return (
                              <td
                                key={j}
                                className={`px-3 py-2 text-right ${isMaxCell ? 'bg-green-200 font-medium' : ''
                                  }`}
                              >
                                {int(v)}
                              </td>
                    ...
                    <tr key={r.profesi}>
                      ...
                      {profesiTPSb.tps.map((tp) => {
                        ...
                        return (
                          <td
                            key={tp}
                            className={`border px-3 py-2 text-right ${isMaxCell ? 'bg-green-200 font-medium' : ''
                              }`}
                          >
                            {int(v)}
                          </td>
        ...
        {/* ===================== 8. KREDIT: USIA × PROFESI × TUJUAN ===================== */}
        <section className="space-y-4 rounded-2xl border bg-white p-6 shadow">
          <h2 className="text-xl font-bold">8. Klasifikasi Usia, Profesi, dan Tujuan Kredit</h2>
          <KreditUsiaProfesiTujuanTable year={tahun ? Number(tahun) : undefined} tp={tp} />
        </section>
        ...
        {/* ===================== 13. TUJUAN PINJAMAN VS KATEGORI PEMBAYARAN ===================== */}
        <section className="space-y-4 rounded-2xl border bg-white p-6 shadow">
          <TujuanPinjamanPembayaranTable year={tahun ? Number(tahun) : undefined} tp={tp} />
        </section>

        {/* ===================== USIA & PROFESI PER TP TABLES ===================== */}
        <section className="space-y-6 rounded-2xl border bg-white p-6 shadow">
          <h2 className="text-2xl font-bold">Klasifikasi per Tempat Pelayanan (TP)</h2>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Usia Pinjaman per TP */}
            <UsiaPerTPTable
              year={tahun ? Number(tahun) : undefined}
              tp={tp}
              title="Klasifikasi Usia Pinjaman per TP"
              endpoint="/api/pinjaman/analytics/usia"
            />

            {/* Usia Kredit per TP dengan Status */}
            <UsiaKreditWithStatusTable
              year={tahun ? Number(tahun) : undefined}
              tp={tp}
            />
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Profesi Simpanan per TP */}
            <ProfesiPerTPTable
              year={tahun ? Number(tahun) : undefined}
              tp={tp}
              title="Klasifikasi Profesi Simpanan Berjangka per TP"
              endpoint="/api/simpanan-berjangka/analytics/profesi-tp"
            />

            {/* Profesi Kredit per TP dengan Status */}
            <ProfesiKreditWithStatusTable
              year={tahun ? Number(tahun) : undefined}
              tp={tp}
            />
          </div>
